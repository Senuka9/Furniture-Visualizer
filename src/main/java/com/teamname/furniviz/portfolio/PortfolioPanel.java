...
public class PortfolioPanel extends JPanel {
    ...
    private Consumer<DesignModel> onDesignSelected; // New: passes design to callback
    
    public PortfolioPanel(Runnable onBackCallback, Runnable onLoadDesign, Consumer<DesignModel> onDesignSelected) {
        this.onBackCallback = onBackCallback;
        this.onLoadDesign = onLoadDesign;
        this.onDesignSelected = onDesignSelected;
    ...
    public PortfolioPanel(Runnable onBackCallback, Runnable onLoadDesign) {
        this(onBackCallback, onLoadDesign, null);
    }
    ...
    private void initUI() {
        ...
        // NORTH: Title and Navigation Buttons
        JPanel topPanel = new JPanel(new BorderLayout());
        topPanel.setBackground(new Color(245, 245, 245));
        topPanel.setPreferredSize(new Dimension(0, 70)); // Set height for buttons to be visible
        ...
        // Navigation buttons panel (right side)
        JPanel navPanel = new JPanel(new FlowLayout(FlowLayout.RIGHT, 10, 10));
        navPanel.setBackground(new Color(245, 245, 245));
        navPanel.setPreferredSize(new Dimension(280, 60)); // Set size for button panel
        
        JButton dashboardButton = new JButton("📊 Dashboard");
        dashboardButton.setFont(new Font("Segoe UI", Font.PLAIN, 12));
        dashboardButton.setPreferredSize(new Dimension(140, 40));
        dashboardButton.setBackground(new Color(76, 175, 80));
        dashboardButton.setForeground(Color.WHITE);
        dashboardButton.setFocusPainted(false);
        dashboardButton.setBorderPainted(false);
        dashboardButton.setOpaque(true);
        dashboardButton.addActionListener(e -> onBackCallback.run());
    ...
    private void sortDesigns() {
        ...
        if ("Date (Newest)".equals(sortBy)) {
            userDesigns.sort((a, b) -> b.getLastModifiedAt().compareTo(a.getLastModifiedAt()));
        } else if ("Date (Oldest)".equals(sortBy)) {
            userDesigns.sort((a, b) -> a.getLastModifiedAt().compareTo(b.getLastModifiedAt()));
        } else if ("Name (A-Z)".equals(sortBy)) {
            userDesigns.sort((a, b) -> a.getDesignName().compareTo(b.getDesignName()));
        } else if ("Name (Z-A)".equals(sortBy)) {
            userDesigns.sort((a, b) -> b.getDesignName().compareTo(a.getDesignName()));
        }
    }
    ...
    private JPanel createDesignCard(DesignModel design) {
        ...
        openBtn.setFont(new Font("Segoe UI", Font.PLAIN, 10));
        openBtn.setPreferredSize(new Dimension(60, 30));
        openBtn.setBackground(new Color(76, 175, 80));
        openBtn.setForeground(Color.WHITE);
        openBtn.setFocusPainted(false);
        openBtn.setBorderPainted(false);
        openBtn.addActionListener(e -> loadDesign(design));
    ...
    private void loadDesign(DesignModel design) {
        try {
            ...
            JOptionPane.showMessageDialog(this, 
                    "Loading design: " + design.getDesignName() + 
                    "\nRoom: " + design.getRoom().getWidth() + "m x " + design.getRoom().getLength() + "m" +
                    "\nFurniture items: " + design.getFurnitureItems().size() +
                    "\n\nNavigating to 2D Editor...",
                    "Design Loaded", 
                    JOptionPane.INFORMATION_MESSAGE);
            
            // If we have a design consumer, use it (new way)
            if (onDesignSelected != null) {
                onDesignSelected.accept(design);
            } else {
            ...
            System.err.println("[ERROR] Error loading design: " + e.getMessage());
            e.printStackTrace();
            JOptionPane.showMessageDialog(this, 
